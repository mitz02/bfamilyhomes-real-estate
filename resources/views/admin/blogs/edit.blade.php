...
<div class="mb-8">
    <div class="flex items-center gap-2 text-xs md:text-sm text-gray-500 mb-4 px-2 md:px-0">
        ...
        <i class="bi bi-chevron-right text-xs"></i>
        <span class="text-orange-600 font-semibold">Edit Post</span>
    </div>
    ...
    <div class="relative bg-gradient-to-br from-blue-900 via-blue-800 to-blue-900 rounded-2xl p-6 md:p-8 shadow-lg overflow-hidden">
        ...
        <div class="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h1 class="text-xl md:text-2xl font-bold text-white">Edit Blog Post</h1>
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="excerpt" rows="3" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all" 
                          placeholder="Brief summary of the blog post (optional)">{{ $blog->excerpt }}</textarea>
            ...
            <div>
                ...
                <div class="mb-4">
                    <img src="{{ asset('storage/' . $blog->featured_image) }}" 
                         ...
                         class="w-full max-w-md rounded-xl ring-2 ring-gray-100 mb-2">
                    <p class="text-xs text-gray-500">Current featured image</p>
                </div>
                @endif
                ...
                <p class="text-xs text-gray-500 mt-1">Leave empty to keep current image. Recommended size: 1200x630px</p>
...
<script>
    document.getElementById('blogForm').addEventListener('submit', async function(e) {
        ...
        try {
            const data = await window.ajax('{{ route("admin.blogs.update", $blog->id) }}', 'POST', formData, true);
            window.toast(data.message, 'success');
            setTimeout(() => {
                window.location.href = data.redirect;
            }, 1000);
        } catch (error) {
            window.toast(error.message || 'Failed to update blog post', 'error');
        } finally {
