...
<div class="mb-8">
    <div class="flex items-center gap-2 text-xs md:text-sm text-gray-500 mb-4 px-2 md:px-0">
        ...
        <span class="text-orange-600 font-semibold">My Profile</span>
    </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <form id="profileForm" class="space-y-6">
                ...
                <div class="flex items-center gap-6 mb-6">
                    <div class="relative">
                        <div class="w-24 h-24 bg-blue-900 rounded-full flex items-center justify-center text-white text-3xl font-bold overflow-hidden ring-4 ring-blue-100">
                            @if(auth()->user()->avatar)
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all" 
                           value="{{ auth()->user()->email }}" required>
                    <p class="text-xs text-gray-500 mt-1">Admins can change their email</p>
                </div>
...
<script>
    ...
    document.getElementById('profileForm').addEventListener('submit', async function(e) {
        ...
        try {
            const data = await window.ajax('{{ route("admin.profile") }}', 'POST', formData);
