...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <form id="profileForm" class="space-y-6">
                ...
                <div>
                    ...
                    @if(auth()->user()->isAdmin())
                        <input type="email" id="email" name="email" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500" 
                               value="{{ auth()->user()->email }}" required>
                        <p class="text-xs text-gray-400 mt-1">Admins can change their email</p>
                    @else
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 font-semibold transition-all text-sm">
                        Cancel
                    </a>
                </div>
            </form>
    ...
    <div class="lg:col-span-1">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold {{ 
                        auth()->user()->isAdmin() ? 'bg-purple-100 text-purple-700' : 
                        (auth()->user()->isAgent() ? 'bg-orange-100 text-orange-700' : 
                        (auth()->user()->isInvestor() ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'))
                    }}">
                ...
                <div>
                    <p class="text-sm text-gray-500 mb-1">Investor Since</p>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mt-6">
            <h3 class="text-lg font-bold text-gray-900 mb-4">Upgrade Options</h3>
            
            @if(!auth()->user()->investor_requested_at)
            <button id="requestInvestorBtn" class="w-full px-5 py-2.5 bg-gradient-to-r from-orange-500 to-yellow-500 text-white rounded-xl hover:from-orange-600 hover:to-yellow-600 font-semibold transition-all text-sm shadow-sm mb-3 inline-flex items-center justify-center gap-2">
                <i class="bi bi-graph-up-arrow"></i>
                Become an Investor
            </button>
            @elseif(!auth()->user()->investor_approved_at)
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-3">
                <p class="text-sm text-gray-700">Investor upgrade pending approval</p>
            </div>
            @endif

            @if(!auth()->user()->agent_approved_at && auth()->user()->role !== 'agent')
            <button id="requestAgentBtn" class="w-full px-5 py-2.5 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 font-semibold transition-all text-sm inline-flex items-center justify-center gap-2">
...
<script>
    function previewAvatar(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                avatarDiv.innerHTML = `<img src="${e.target.result}" class="w-24 h-24 rounded-full object-cover">`;
            };
    ...
    document.getElementById('profileForm').addEventListener('submit', async function(e) {
        ...
        try {
            const data = await window.ajax('{{ route("dashboard.profile") }}', 'POST', formData);
